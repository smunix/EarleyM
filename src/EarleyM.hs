...
instance Applicative Gram where pure = Ret; (<*>) = ap
...
newtype Lang t a = Lang {runLang :: NT t -> (a, [Rule])}

instance Functor (Lang t) where fmap = liftM

instance Applicative (Lang t) where pure a = Lang $ const (a, []); (<*>) = ap
...
produce nt gram = Lang $ const ((), [Rule nt gram])
...
loop startNT tokenNT config rules pos s xs = case xs of
    [] ->
        case lookChan s (startNT, 0) of
            Nothing -> error "startChan missing"
            Just chan -> do
                let results = do (a, p) <- Pipe.elems chan; guard $ p == pos; pure a
                case results of
                    [] ->
