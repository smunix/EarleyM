...
lang = do
  ...
  let var = do Var <$> ident
  let num = do Num <$> digits
  ...
  exp <- fix "exp" $ \exp -> do
    ...
    let atomO = (,Open) <$> alts [var, num]
    let atomC = (,Closed) <$> parenthesized (lambdarized exp)
    ...
    produce app' $
      alts
        ...
        , do
            ...
            (b, oc2) <- case (oc1, gap) of
              (Open, False) -> atomC
              (_, _)        -> atom -- context sensitive grammar here
            return (App a b, oc2)
        ]
    ...
    let addition =
          alts
            [ application
            , do a <- exp; ws; symbol '+'; ws; Add a <$> exp -- grammar is deliberately ambiguous here
            ]
    return addition
